...
bool ProjectSerializer::save(const Project& project, const juce::File& file)
{
    ...
    for (const auto& pat : project.patterns)
    {
        ...
        for (int ch = 0; ch < Pattern::kMaxChannels; ++ch)
        {
            ...
            chEl->setAttribute("spLfoWaveform", sp.lfoWaveform);
            chEl->setAttribute("spLfoFreeRun",  sp.lfoFreeRun ? 1 : 0);
            chEl->setAttribute("spLfoFadeIn",   (double)sp.lfoFadeIn);
            chEl->setAttribute("spPulseWidth",  (double)sp.pulseWidth);
            ...
            chEl->setAttribute("spDriftDepth",      (double)sp.driftDepth);
    ...
    for (const auto& preset : project.customSynthPresets)
    {
        ...
        pEl->setAttribute("lfoWaveform",  preset.params.lfoWaveform);
        pEl->setAttribute("lfoFreeRun",   preset.params.lfoFreeRun ? 1 : 0);
        pEl->setAttribute("lfoFadeIn",    (double)preset.params.lfoFadeIn);
        pEl->setAttribute("pulseWidth",   (double)preset.params.pulseWidth);
        ...
        pEl->setAttribute("driftDepth",      (double)preset.params.driftDepth);
...
bool ProjectSerializer::load(juce::File& file, Project& projectOut)
{
    ...
    if (auto* patternsEl = xml->getChildByName("Patterns"))
    {
        for (auto* patEl : patternsEl->getChildIterator())
        {
            ...
            for (auto* chEl : patEl->getChildIterator())
            {
                ...
                sp.lfoWaveform = chEl->getIntAttribute("spLfoWaveform", 0);
                sp.lfoFreeRun  = chEl->getIntAttribute("spLfoFreeRun",  0) != 0;
                sp.lfoFadeIn   = (float)chEl->getDoubleAttribute("spLfoFadeIn", 0.0);
                sp.pulseWidth  = (float)chEl->getDoubleAttribute("spPulseWidth", 0.5);
                ...
                sp.driftDepth        = (float)chEl->getDoubleAttribute("spDriftDepth", 0.3);
    ...
    if (auto* synthPresetEl = xml->getChildByName("CustomSynthPresets"))
    {
        for (auto* pEl : synthPresetEl->getChildIterator())
        {
            ...
            preset.params.lfoWaveform  = pEl->getIntAttribute("lfoWaveform",  0);
            preset.params.lfoFreeRun   = pEl->getIntAttribute("lfoFreeRun",   0) != 0;
            preset.params.lfoFadeIn    = (float)pEl->getDoubleAttribute("lfoFadeIn",    0.0);
            preset.params.pulseWidth   = (float)pEl->getDoubleAttribute("pulseWidth",   0.5);
            ...
            preset.params.driftDepth      = (float)pEl->getDoubleAttribute("driftDepth",      0.3);
